...
remove_legacy_1password_identity_agent() {
  local ssh_config="$1"
  local tmp_file
  local rc

  [[ -e "$ssh_config" ]] || return 1

  tmp_file="$(mktemp)" || return 1
  set +e
  awk '
    /^[[:space:]]*IdentityAgent[[:space:]]+"?~\/Library\/Group Containers\/2BUA8C4S2C\.com\.1password\/t\/agent\.sock"?[[:space:]]*$/ {
      removed=1
      next
    }
    /^[[:space:]]*IdentityAgent[[:space:]]+"?\/Users\/[^"]+\/Library\/Group Containers\/2BUA8C4S2C\.com\.1password\/t\/agent\.sock"?[[:space:]]*$/ {
      removed=1
      next
    }
    { print }
...
ensure_ssh_config_identity_agent() {
  ...
  if remove_legacy_1password_identity_agent "$ssh_config"; then
    status "ssh config legacy IdentityAgent" "removed"
  fi
  ...
  if ! op item list --categories "SSH Key" --format json 2>/dev/null \
    | jq -r --arg host "$host_alias" --arg priority "$peer_priority" '
      ...
      | "# Generated by dotfiles scripts/setup-1password-dev.zsh.\n"
        ...
        + "# Use SSH Bookmarks or per-host IdentityFile + IdentitiesOnly to select keys.\n"
        + "# Do not truncate this list: omitted keys cannot authenticate even with a bookmark.\n\n"
        + (map("[[ssh-keys]]\nitem = " + (.id | @json) + "\nvault = " + (.vault.id | @json) + "\n") | join("\n"))
    ' > "$tmp_file"; then
    rm -f "$tmp_file"
