...
Events deduplicate conservatively within a run using session identity, the sorted unique set of canonical primary and mirrored-record SHA-256 hashes, and the occurrence ordinal among equal hash sets in that host's ordered events. Collector v3 includes these canonical digests so JSON whitespace/key ordering and swapping which mirrored envelope is primary do not change identity. Raw byte hashes remain in source references for detail verification and are unioned in the merged record. Older v2 bundles require recollection; their raw-only identities cannot establish the same contract. Identical repetitions remain separate; copied events with the same hash set retain every host/reference provenance. Different hash sets do not merge. Every declared session source file and every primary, mirror and paired-call reference must name a `.jsonl` file in the session's declared provenance. This is not durable per-session offset reconciliation, and separate runs retain their own evidence.
