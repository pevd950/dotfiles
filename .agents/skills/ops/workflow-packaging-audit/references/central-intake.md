...
Events that are unique within each contributing complete input deduplicate within a run using session identity and the sorted unique set of canonical primary and mirrored-record SHA-256 hashes. If any admitted host reports repeated equal digest sets or contributes partial evidence for that group, every occurrence in the group stays host-local, including occurrences admitted earlier. A local ordinal distinguishes occurrences only inside one host; it is never evidence that two hosts captured the same physical repetition. The ledger labels this identity scope explicitly. This conservatively retains potentially duplicated evidence when occurrence correspondence is unknown; unique-in-input matching still does not establish complete source history. Collector v3 includes these canonical digests so JSON whitespace/key ordering and swapping which mirrored envelope is primary do not change identity. Raw byte hashes remain in source references for detail verification and are unioned in the merged record. Older v2 bundles require recollection; their raw-only identities cannot establish the same contract. Identical repetitions remain separate per host; safely merged unique records retain every host/reference provenance. Different hash sets do not merge. Every declared session source file and every primary, mirror and paired-call reference must name a NUL-free `.jsonl` path in the session's declared provenance. Every reference end offset must fit both reported scanned bytes and the declared byte budget. Mirrored messages require exactly one mirror reference and two distinct canonical digests; non-mirrored events carry one digest. This is not durable per-session offset reconciliation, and separate runs retain their own evidence.
...
Counters exceeding declared collector budgets retain the input as partial evidence but prevent acknowledgment. Each record-derived category counter is bounded by records read, and the combined excluded-file counters cannot exceed files read. Emitted sessions cannot outnumber files actually read, and read files cannot outnumber discovered files; empty files need not contain records. Event kinds have separate required fields and candidate signals, using the collector's signal definitions. Paired calls cannot occur after their result, and their before-window marker must agree with the exclusive lower bound. The collector record limit cannot exceed the shared 1 MiB detail ceiling; primary, mirror and paired-call references must each fit the declared record limit. Approval sidecars cannot appear as emitted sessions. Unknown source classes and untimestamped events retain partial evidence and cannot be acknowledged even if their source gaps were removed. Root-directory gaps remain path-only metadata and are not required to name `.jsonl` files.
