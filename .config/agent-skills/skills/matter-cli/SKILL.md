...
- Use the local CLI directly: `/Users/pablovalero/.matter/bin/matter`.
...
```bash
/Users/pablovalero/.matter/bin/matter items list --status queue --order library_position --limit 20
```

Use inbox order for incoming feed/newsletter work:

```bash
/Users/pablovalero/.matter/bin/matter items list --status inbox --order inbox_position --limit 20
```

Use search before pulling full article bodies:

```bash
/Users/pablovalero/.matter/bin/matter search "query" --type items --limit 10
```

Pull full article text only when needed for summarization, classification, or synthesis:

```bash
/Users/pablovalero/.matter/bin/matter items get itm_abc123 --include markdown
```
...
```bash
/Users/pablovalero/.matter/bin/matter tags list --plain
/Users/pablovalero/.matter/bin/matter tags add --item itm_abc123 ai
/Users/pablovalero/.matter/bin/matter tags remove --item itm_abc123 old-tag
/Users/pablovalero/.matter/bin/matter tags rename old-tag new-tag
```
...
```bash
python3 /Users/pablovalero/.codex/skills/matter-cli/scripts/matter_link.py itm_8J1dX
```
...
```text
/Users/pablovalero/.codex/automations/matter-reading-queue/automation.toml
/Users/pablovalero/.codex/automations/matter-reading-queue/memory.md
```

When editing that automation, validate the TOML after changes:

```bash
python3 -c 'import tomllib, pathlib; tomllib.loads(pathlib.Path("/Users/pablovalero/.codex/automations/matter-reading-queue/automation.toml").read_text()); print("automation toml ok")'
```
